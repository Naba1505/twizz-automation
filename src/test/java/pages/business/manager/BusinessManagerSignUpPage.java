    ...
    public void selectBirthDate() {
        ...
        try {
            ...
            if (currentMonth.count() > 0) {
                ...
                page.waitForTimeout(300);
                logger.info("[Business Manager] Clicked current month to open navigation");
            }
        } catch (Exception e) {
            logger.debug("[Business Manager] Could not click current month: {}", e.getMessage());
        }
        ...
        for (String month : januaryVariations) {
            try {
                ...
                if (monthElement.count() > 0 && safeIsVisible(monthElement.first())) {
                    ...
                    page.waitForTimeout(300);
                    januaryFound = true;
                    logger.info("[Business Manager] Found and clicked January: {}", month);
                    break;
                }
            } catch (Exception e) {
                logger.debug("[Business Manager] January variation '{}' not found: {}", month, e.getMessage());
            }
        }
        ...
        if (!januaryFound) {
            try {
                ...
                if (anyMonth.count() > 0) {
                    ...
                    page.waitForTimeout(300);
                    logger.info("[Business Manager] Clicked first available month as fallback");
                }
            } catch (Exception e) {
                logger.debug("[Business Manager] Fallback month selection failed: {}", e.getMessage());
            }
        }
        ...
        logger.info("[Business Manager] Selected birth date: 1995-01-01");
    }
